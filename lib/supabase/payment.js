 ...
 */
export async function ensureUserPayment(supabase, user, overridePlan = null, businessRequirement = null) {
  const userId = user?.id;
  if (!userId) return null;

  try {
    const requirement = businessRequirement || user.user_metadata?.business_requirement || null;
    ...
    if (existing) {
      if (requirement && !existing.business_requirement) {
        await supabase
          .from("tb_payment")
          .update({ business_requirement: requirement })
          .eq("uid", userId);
        existing.business_requirement = requirement;
      }
      return existing;
    }
